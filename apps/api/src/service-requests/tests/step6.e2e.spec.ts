...
process.env.JWT_ACCESS_SECRET =
  process.env.JWT_ACCESS_SECRET ??
  'access-secret-for-step6-tests-at-least-32-chars';
...
describe('ServiceRequestsController (Step 6, real database)', () => {
  ...
  const createUserWithRole = async (role: UserRole) => {
    const user = await prisma.user.create({
      data: {
        mobile: `9600${createdUserIds.length}${role.length}${suffix}`.slice(
          0,
          15,
        ),
  ...
  it('gets an owned request and returns 404 for another customer request', async () => {
    ...
    const own = await request(
      ...
      .get(
        `/api/v1/customers/me/service-requests/${requestId}`,
      )
      .set(
        'Authorization',
        `Bearer ${customerBToken}`,
  ...
  it('updates an owned request only', async () => {
    ...
    expect(updated.body.title).toBe(
      'After update',
    );
    expect(updated.body.location).toBe(
      'New location',
    );
  ...
  it('keeps historical requests valid after skill deactivation', async () => {
    ...
    expect(after.body.skill.id).toBe(
      activeSkillId,
    );
