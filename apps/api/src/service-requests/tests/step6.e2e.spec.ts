...
process.env.JWT_REFRESH_SECRET =
  process.env.JWT_REFRESH_SECRET ?? 'refresh-secret-for-step6-tests-at-least-32-ch';
 ...
 * Every record created here uses a unique suffix and is cleaned up in
 * afterAll so the existing Step 1-5 data is never disturbed.
 */
describe('ServiceRequestsController (Step 6, real database)', () => {
  ...
  beforeAll(async () => {
    ...
    // Guards resolved through DI so their own JwtService/AuthorizationService work.
    app.useGlobalGuards(
    ...
    await prisma.customer.create({ data: { userId: customerA.id } });
    ...
    await prisma.worker.create({ data: { userId: worker.id } });
  ...
  it('returns 403 for a COOPERATIVE_ADMIN on create', async () => {
    const res = await request(app.getHttpServer())
      ...
      .send({ skillId: activeSkillId, title: 'Admin attempt' });
    expect(res.status).toBe(403);
  });

  it('lets a customer create a service request (status OPEN)', async () => {
    const res = await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
  ...
  it('rejects creation with an inactive skill', async () => {
    const res = await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
      .set('Authorization', `Bearer ${customerAToken}`)
      .send({ skillId: inactiveSkillId, title: 'Should not pass' });
  ...
  it('rejects creation with an unknown skill', async () => {
    const res = await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
      .set('Authorization', `Bearer ${customerAToken}`)
      .send({ skillId: '00000000-0000-4000-8000-000000000000', title: 'Nope' });
    expect(res.status).toBe(404);
  });
  ...
  it('rejects invalid payloads with 400 including forged customerId', async () => {
    const missingTitle = await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
      .set('Authorization', `Bearer ${customerAToken}`)
      .send({ skillId: activeSkillId });
    expect(missingTitle.status).toBe(400);

    const badUuid = await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
      .set('Authorization', `Bearer ${customerAToken}`)
      .send({ skillId: 'not-a-uuid', title: 'Bad uuid' });
  ...
  it('lists only the authenticated customer requests, newest first', async () => {
    await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
      .set('Authorization', `Bearer ${customerAToken}`)
      .send({ skillId: activeSkillId, title: 'Second request' });
  ...
  it('gets an owned request and 404s another customer request', async () => {
    const created = await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
      .set('Authorization', `Bearer ${customerBToken}`)
      .send({ skillId: activeSkillId, title: 'B request' });
  ...
  it('updates an owned request only', async () => {
    const created = await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
      .set('Authorization', `Bearer ${customerAToken}`)
      .send({ skillId: activeSkillId, title: 'Before update' });
    const requestId = created.body.id as string;

    const updated = await request(app.getHttpServer())
      .patch(`/api/v1/customers/me/service-requests/${requestId}`)
      .set('Authorization', `Bearer ${customerAToken}`)
      .send({ title: 'After update', location: 'New location' });
  ...
  it('cancels an owned request', async () => {
    const created = await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
      .set('Authorization', `Bearer ${customerAToken}`)
      .send({ skillId: activeSkillId, title: 'To cancel' });
    const requestId = created.body.id as string;
  ...
  it('keeps historical requests valid after skill deactivation', async () => {
    const created = await request(app.getHttpServer())
      .post('/api/v1/customers/me/service-requests')
      .set('Authorization', `Bearer ${customerAToken}`)
      .send({ skillId: activeSkillId, title: 'Survives deactivation' });
    const requestId = created.body.id as string;
